...
jQuery(function ($) {
	...
	const goToDraftPage = function ( postData = {} ) {
		...
		$.post(visualizerSetupWizardData.ajax.url, postData, function (res) {
			...
			$('.redirect-popup').find('h3.popup-title').text(res.message);
			$('.redirect-popup').show();
		...
		$.ajax({
			...
			success: function (data) {
				...
				if (1 === data.success) {
					...
					$error.empty().append( $('<p>').text( data.message ) ).removeClass('hidden');
					$spinner.removeClass('is-active');
	...
	$(document).on( 'click', '.vz-subscribe', function (e) {
		...
		installPluginsSequentially(slugs.slice(), function (ok, message) {
			if (!ok) {
				if (message) {
					$error.empty().append( $('<p>').text( message ) ).removeClass('hidden');
				}
